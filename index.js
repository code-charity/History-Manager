...
function openDB(callback) {
    chrome.storage.local.get('version', function(items) {
        ...
        chrome.storage.local.set({
            ...
            var request = indexedDB.open('storage', version);
            ...
            request.onupgradeneeded = function() {
                ...
                if (!DB.objectStoreNames.contains('domains')) {
                    var object = DB.createObjectStore('domains', {keyPath: 'domain'});
                ...
                if (!DB.objectStoreNames.contains('pages')) {
                    var object = DB.createObjectStore('pages', {keyPath: 'id'});
                ...
                if (!DB.objectStoreNames.contains('params')) {
                    var object = DB.createObjectStore('params', {keyPath: 'domain'});
                ...
                if (!DB.objectStoreNames.contains('transitions')) {
                    var object = DB.createObjectStore('transitions', {keyPath: 'transition'});
...
function getDBData(object_name, index_name, sort, offset, callback, max) {
    var transaction = DB.transaction(object_name, 'readonly'),
        pages = transaction.objectStore(object_name),
        index = index_name ? pages.index(index_name) : pages,
        data = [],
        counter = 0;
    ...
    index.count().onsuccess = function(event) {
        ...
        index.openCursor(null, sort === 'desc' ? 'prev' : 'next').onsuccess = function(event) {
            var cursor = event.target.result;

            counter++;

            if (cursor && (data.length < 100 || max === false)) {
                if (counter > offset) {
                ...
                callback(data, count, object_name);
            }
        };
    };
}
...
function renderTables() {
    ...
    getDBData('domains', 'visitCountIndex', 'desc', 0, function(data, count) {
        var categories = [];
        ...
        for (var key in CATEGORIES) {
            ...
            for (var link in category) {
                for (var key2 in data) {
                    var domain = data[key2].domain;

                    if (domain.indexOf(link) !== -1) {
                        category[link] = data[key2].visitCount;

                        category.visitCount += data[key2].visitCount;
                    }
                }
    ...
    getDBData('domains', 'visitCountIndex', 'desc', 0, function(data, count) {
        ...
        table_pages.data = data;
        table_pages.count = count;

        table_pages.update();
    });
                
    getDBData('pages', 'visitCountIndex', 'desc', 0, function(data, count) {
...
satus.locale.load('_locales/en/messages.json', function () {
    ...
    satus.storage.load(null, function (items) {
        if (items['search-autofocus-mode'] !== false && location.href.indexOf('?loaded') === -1) {
        ...
        openDB(function() {
            getDBData('transitions', null, 'asc', 0, function(transitions) {
                TRANSITIONS = transitions || {};

                getHistoryItems(function(items) {
                    if (items.length > 0) {
                        var transaction = DB.transaction('pages', 'readwrite'),
                        ...
                        for (var i = 0, l = items.length; i < l; i++) {
                            ...
                            chrome.history.getVisits({url: item.url}, function(visitItems) {
                                for (var i = 0, l = visitItems.length; i < l; i++) {
                                    ...
                                    if (!TRANSITIONS[visitItem.transition]) {
                                        TRANSITIONS[visitItem.transition] = {
                                            transition: visitItem.transition,
                                            visitCount: 0
                                        };
                                    }

                                    TRANSITIONS[visitItem.transition].visitCount++;
                                }
                                ...
                                if (threads === 0) {
                                    var transaction = DB.transaction('domains', 'readwrite'),
                                        object = transaction.objectStore('domains');

                                    for (var key in domains) {
                                        domains[key].domain = key;

                                        object.add(domains[key]);
                                    }

                                    var transaction = DB.transaction('params', 'readwrite'),
                                        object = transaction.objectStore('params');

                                    for (var key in params) {
                                        object.add(params[key]);
                                    }

                                    var transaction = DB.transaction('transitions', 'readwrite'),
                                        object = transaction.objectStore('transitions');

                                    for (var key in transitions) {
                                        object.add(transitions[key]);
                                    }
        ...
        document.querySelector('.satus-footer__storage-size').textContent = 'Storage: ' + (JSON.stringify(items).length * 2 / 8e+6).toFixed(2) + ' MB';

        var favic = document.querySelector('.satus-button--search-options');

        if (favic && items['search-engine']) {
            favic.style.backgroundImage = 'url(' + SEARCH_ENGINE[items['search-engine']].favicon + ')';

            document.querySelector('.search-field').placeholder = 'Search ' + SEARCH_ENGINE[items['search-engine']].name + ' or type a URL'; 
        }
    });
});
